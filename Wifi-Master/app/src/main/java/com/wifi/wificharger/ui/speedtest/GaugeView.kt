...
class GaugeView : View {
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        val a: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.GaugeView, 0, 0)
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        rect!![(width - 2 * strokeWidth) / 2 - radius + strokeWidth, (height - 2 * strokeWidth) / 2 - radius + strokeWidth, (width - 2 * strokeWidth) / 2 - radius + strokeWidth + w] =
            (height - 2 * strokeWidth) / 2 - radius + strokeWidth + h
        ...
        canvas.drawArc(
            rect!!,
            startAngle.toFloat(),
            (startAngle + value * (abs(angles).toDouble() / maxValue) - startAngle).toFloat(),
            false,
            paint!!
        )
    }
