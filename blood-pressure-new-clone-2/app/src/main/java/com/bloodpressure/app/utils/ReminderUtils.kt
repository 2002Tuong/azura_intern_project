...
import com.bloodpressure.app.receiver.FullScreenReminderReceiver
...
object ReminderUtils {
    fun scheduleFullscreenReminder(context: Context, requestCode: Int, timeInMs: Long) {
        val intent =
            Intent(context, FullScreenReminderReceiver::class.java)
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            alarmManager.setAlarmClock(
                AlarmManager.AlarmClockInfo(timeInMs, broadcast),
                broadcast
            )
        }
    ...
    fun scheduleNextFullScreenReminder(context: Context) {
        ...
        val reminderTimeMs =
            if (BuildConfig.DEBUG) Calendar.getInstance().timeInMillis + 5_000L else calendar.timeInMillis
