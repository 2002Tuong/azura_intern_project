...
class BubbleNavigationConstraintView : ConstraintLayout, View.OnClickListener, BubbleNavigation {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        if (attrs != null) {
            ...
            mode = try {
                ta.getInteger(
                    R.styleable.BubbleNavigationConstraintView_bnc_mode,
                    mode
                )
            } finally {
                ta.recycle()
        ...
        if (mode >= 0 && mode < DisplayMode.values().size) displayMode = DisplayMode.values()[mode]
    ...
    private fun updateChildNavItems() {
        ...
        createChains()
        if (curTypeface != null) setTypeface(curTypeface)
    ...
    private fun setInitialActiveState() {
        ...
        if (!loadPreviousState) {
            for (i in bubbleNavItems!!.indices) {
                if (bubbleNavItems!![i].active && !foundActiveElement) {
