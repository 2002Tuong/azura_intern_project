...
public class GPUPlayerRenderer extends GlFrameBufferObjectRenderer implements SurfaceTexture.OnFrameAvailableListener {
    ...
    public void onSurfaceCreated(final EGLConfig config) {
        ...
        handler.post(() -> simpleExoPlayer.setVideoSurface(surface));
    ...
    void cropFrames(int videoWidth, int videoHeight, int x, int y) {
    ...
    public void onDrawFrame(final GlFramebufferObject fbo) {
        ...
        if (glFilter != null) {
            fbo.enable();
            GLES20.glClear(GL_COLOR_BUFFER_BIT);
            if (glFilter instanceof FilterTimable) {
                ((FilterTimable) glFilter).setTime(count);
            }
