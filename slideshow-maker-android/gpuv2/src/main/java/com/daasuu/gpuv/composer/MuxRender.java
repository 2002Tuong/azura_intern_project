...
class MuxRender {
    ...
    void onSetOutputFormat() {
        ...
        // Check if the remaining space in the ByteBuffer is enough for the incoming data
        if (byteBuffer.remaining() < bufferInfo.size) {
            // Increase the size of the ByteBuffer if necessary
            int newSize = byteBuffer.capacity() + (bufferInfo.size - byteBuffer.remaining());
            ByteBuffer newByteBuffer = ByteBuffer.allocateDirect(newSize).order(ByteOrder.nativeOrder());
            byteBuffer.flip();
...
//        byteBuf.limit(bufferInfo.offset + bufferInfo.size);
//        byteBuf.position(bufferInfo.offset);
//        ByteBuffer buffer = ByteBuffer.allocateDirect(bufferInfo.size).order(ByteOrder.nativeOrder());
//        buffer.put(byteBuf);
//        sampleInfoList.add(new SampleInfo(sampleType, bufferInfo.size, bufferInfo));
//    }
