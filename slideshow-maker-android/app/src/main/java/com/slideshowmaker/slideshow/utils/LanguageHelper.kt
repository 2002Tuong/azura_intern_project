...
import com.slideshowmaker.slideshow.utils.extentions.moveItemToPosition
import java.util.Locale

object LanguageHelper {
    private var deviceLocale: Locale? = null
    private val supportedLanguageList = listOf(
        LanguageModel("en", R.string.lang_english, R.drawable.icon_flag_england, true),
    ...
    fun getSupportedLanguage(): List<LanguageModel> {
        ...
        supportDeviceLanguages.forEach {
            it.isChoose = it.code == SharedPreferUtils.langCode
        }
        return supportDeviceLanguages
    }

    private fun saveLocale(lang: String) {
        SharedPreferUtils.setLanguageCode(lang)
    }

    fun loadLocale(context: Context) {
        val lang: String = SharedPreferUtils.langCode
        if (lang == "") {
